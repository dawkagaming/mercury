...
/**
 * CRC-5, polynomial x^5 + x^4 + x^2 + 1 (0x15 in normal form, no implicit leading bit shown).
 * Non-reflected, init=1 (same calling convention as crc6_0X6F).
 * Usage: tbl_idx = (crc << 3) ^ byte;  crc = crc5_table[tbl_idx] & 0x1f
 */
...
uint8_t crc5_0X15(uint8_t crc, const uint8_t *data, int data_len)
{
    while (data_len--)
    {
        crc = crc5_table[((crc << 3) ^ *data) & 0xff] & 0x1f;
        data++;
    }
    return crc & 0x1f;
}
